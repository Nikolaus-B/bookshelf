import{l as f,a as e,b as n,c as t,d as c}from"./dark-theme-626332ef.js";const g="data:image/png;base64,",I="data:image/png;base64,",d="/bookshelf/assets/img-book-Mob1x-1fe4bd25.png",B="/bookshelf/assets/img-book-Mob2x-261d5249.png";document.addEventListener("DOMContentLoaded",f.onDOMContentLoaded);const b=document.querySelector(".book-list"),s="books";function k(){const o=`
  <div class="empty-list">
      <p class="empty-list-text">
        This page is empty, add some books and proceed to order.
      </p>
      <img
         class="empty-list-img"
         src="${d}"
         srcset="
         ${d} 1x,
         ${B} 2x
         "
        alt="books"
        width="265"
        height="198"
         />
      </div>`;b.innerHTML=o}function m(){const i=(JSON.parse(localStorage.getItem(s))||[]).map(({_id:l,book_image:A,title:a,description:r,author:h,amazon_product_url:p,list_name:x,buy_links:w})=>`
      <li class="book_li book js-card" data-book-id="${l}">
        <div class="book_li-img">
              <img class="book_li-i" src="${A||"../img/def-placeholder.png"}" alt="${a}" />
            </div>
            <div class="book_li-info">
              <div class="book_li-header">
                <button class="book_li-btn js-card__delete">
                  <img
                    class="book_li-icon js-card__delete"
                    src="${g}"
                    srcset="
                      ${g} 1x,
                      ${I} 2x
                    "
                    alt="basket"
                    width="16"
                    height="16"
                  />
                </button>
                <h2 class="book_li-title">${a.trim()}</h2>
                <h3 class="book_li-category">${x.trim()}</h3>
              </div>
              <p class="book_li-description">
                ${r.trim()}
              </p>
              <div class="book_li-footer">
                <p class="book_li-author">${h.trim()}</p>
                <ul class="book-links">
                  <li>
                    <a class="book_link" href="${p}" aria-label="amazon" target="_blank"
                   rel="noopener noreferrer">
                      <img
                        class="book_link-amazon"
                        src="${e}"
                        srcset="${e} 1x,
                        ${n} 2x"
                        alt="amazon"
                        width="32"
                        height="11"
                      />
                      <img
                        class="book_link-amazon hovered"
                        src="${e}"
                        srcset="${e} 1x,
                        ${n} 2x"
                        alt="amazon"
                        width="32"
                        height="11"
                      />
                    </a>
                  </li>
                  <li>
                   <a class="book_link" href="${w[1].url}" aria-label="apple-book" target="_blank"
                   rel="noopener noreferrer">

                      <img
                        class="book_link-book"
                        src="${t}"
                        srcset="${t} 1x,
                        ${c} 2x"
                        alt="book"
                        width="16"
                        height="16"
                      />
                      <img
                        class="book_link-book hovered"
                        src="${t}"
                        srcset="${t} 1x,
                        ${c} 2x"
                        alt="book"
                        width="16"
                        height="16"
                      />
                    </a>
                  </li>
                </ul>
              </div>
            </div>
      </li>
    `).join("");i.length===0?k():b.innerHTML=i}function C(o){if(o.target.classList.contains("js-card__delete")){const l=o.target.closest(".js-card").dataset.bookId,a=(JSON.parse(localStorage.getItem(s))||[]).filter(r=>r._id!==l);localStorage.setItem(s,JSON.stringify(a)),m()}}window.addEventListener("click",o=>C(o));window.onload=()=>{localStorage.getItem(s)!==null||localStorage.getItem(s)!=[]?m():k()};
